...
class MySQL
{
	...
	public function conBDPDO()
		...
		public function getVendidos(){
			$vendidos = 0;
			try {
				$strQuery = "SELECT SUM(telefono) as vendidos FROM t_personasd";
				if ($this->conBDPDO()) {
		...
		public function getAlmacen(){
			$almacen = 0;
			try {
				$strQuery = "SELECT SUM(nombre) as enAlmacen FROM t_personasd";
				if ($this->conBDPDO()) {
					$pQuery = $this->oConBD->prepare($strQuery);
					$pQuery->execute();
		...
		public function getIngresos(){
			$ingreso = 0;
			try {
				$strQuery = "SELECT SUM(precio) as ingresos FROM t_personasd";
				if ($this->conBDPDO()) {
					$pQuery = $this->oConBD->prepare($strQuery);
					$pQuery->execute();
		...
		public function getDatosGrafica()
		{
            ...
            ,DATE_FORMAT(email, '%Y-%m-%d') as email FROM t_personasd GROUP BY DATE_FORMAT(email, '%Y-%m-%d')";
